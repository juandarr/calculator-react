...
KeyPad.defaultProps = { keys: keys, ids: ids };
    ...
    if (this.props.keysIntegers.includes(currentKey) || currentKey === 'PI') {
      ...
      if (this.state.previousKey === '=') {
        ...
        if (currentKey === '-') {
          ...
          this.setState((state) => {
            const lastElem = state.formula.split('').reverse().find((input, idx) => '1234567890.'.includes(input));
...
Calculator.defaultProps = { keysIntegers: keysIntegers, keysOperations: keysOperations, codeToPad: codeToPad };
